...
// Full bug-report detail shown directly inside the detail panel (above the
// SMS-style email thread). Shows the description, metadata fields and large
// attachment thumbnails, plus the ability to add more attachments.
export default function BugReportDetails({ bug, onUpdated }) {
  ...
  return (
    <div className="px-4 pt-4">
      <div className="rounded-2xl bg-white/70 dark:bg-white/10 border border-white/60 dark:border-white/15 p-4 space-y-4">
        {bug.description && (
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2.5">
            {images.map((url, i) =>
              isImg(url) ? (
                <button key={i} type="button" onClick={() => setLightboxIndex(imgUrls.indexOf(url))}>
                  <img src={url} alt="" className="w-28 h-28 rounded-xl object-cover border border-orange-200/60 dark:border-white/15 hover:opacity-90 transition-opacity" />
                </button>
              ) : (
                <a key={i} href={url} target="_blank" rel="noopener noreferrer"
                  className="w-28 h-28 rounded-xl flex flex-col items-center justify-center gap-1.5 bg-orange-50 dark:bg-orange-500/15 text-orange-700 dark:text-orange-200 text-xs font-medium hover:bg-orange-100 transition-colors border border-orange-200/60 dark:border-white/15">
            ...
            <label className="w-28 h-28 rounded-xl flex flex-col items-center justify-center gap-1.5 border-2 border-dashed border-orange-300/60 dark:border-white/20 text-orange-500 dark:text-white/60 cursor-pointer hover:bg-orange-50 dark:hover:bg-white/5 transition-colors">
