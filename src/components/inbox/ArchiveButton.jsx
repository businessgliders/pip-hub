...
export default function ArchiveButton({ threads, onArchive }) {
  ...
  return (
    <button
      onClick={handleClick}
      disabled={busy}
      title="Archive this month & older"
      className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-full text-xs font-semibold bg-white/60 dark:bg-white/10 text-pink-700 dark:text-white/80 hover:bg-white/80 dark:hover:bg-white/20 transition-colors disabled:opacity-60"
    >
