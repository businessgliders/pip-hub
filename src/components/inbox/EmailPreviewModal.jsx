...
function Row({ label, value }) {
  ...
  return (
    <div className="flex gap-2 text-xs">
      <span className="w-12 shrink-0 font-medium text-slate-400">{label}</span>
...
export default function EmailPreviewModal({ message, open, onClose }) {
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-3xl max-h-[85vh] overflow-hidden flex flex-col bg-white p-0">
        <DialogHeader className="px-6 pt-6 pb-4 border-b border-slate-100">
          <DialogTitle className="text-base text-slate-900">{message.subject || "(no subject)"}</DialogTitle>
