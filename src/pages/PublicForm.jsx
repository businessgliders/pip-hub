...
import PublicFormField from "@/components/inbox/forms/PublicFormField";

const ACCENT = "#7d2235";

// Public form-fill page. Opened via /form?token=XXXX from an invite email.
// Loads & submits through token-validated backend functions (no login needed).
export default function PublicForm() {
  ...
  useEffect(() => {
    ...
    base44.functions.invoke("getPublicForm", { token })
      .then((res) => {
        const d = res?.data || {};
  ...
  const submit = async (e) => {
    ...
    setError("");
    try {
      const res = await base44.functions.invoke("submitPublicForm", { token, answers });
      const d = res?.data || {};
      if (d.error) throw new Error(d.error);
      setDone(true);
    } catch (err) {
    ...
    <div className="min-h-screen w-full flex items-start sm:items-center justify-center p-4 py-10"
      ...
      <div className="w-full max-w-lg">
        {loading ? (
          ...
          <Card>
            <h1 className="text-2xl font-bold text-pink-900">{data.form.name}</h1>
